...
def main(args):
    ...
    if 'train' in args:
        ...
        for imgFileName in os.listdir('images'): # for each image in 'images' directory:
            ...
            img = croppedFaceFromImg(cv2.imread('images/' + imgFileName), image_size)
        ...
        dataset = getAugmentedData(images, labels, image_size, 1000)
    ...
    if "run" in args:
        ...
        while True:
            ...
            croppedImg = croppedFaceFromImg(frame, image_size) 
            if croppedImg is None:
                continue

            croppedImg = croppedImg / 255.0 # normalize values to range of 0-1
            
            cv2.imshow('test', croppedImg)
            cv2.waitKey(10)

            croppedImg = croppedImg.reshape((1,) + croppedImg.shape)
            ...
            print(int(predicted_class_index), labels[str(predicted_class_index)])
